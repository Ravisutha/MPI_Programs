...
int guassian_elimination (int total_rows, int cols, int rank, int size, double **array)
{
	...
	for (i = initial_value; i < total_rows; i++)
	{
		...
		{
			//print_array (&(array[j][0]), cols);
			//printf ("(rank = %d)current row = %d, expected row = %d\n", rank, row_number, j);

			/* If the jth row doesn't belongs the same process */
			if (((rank - j) % size) != 0)
			{
				printf ("(rank = %d)Receiving row %d from process %d\n", rank, j, pre_process);
				...
				{
					//printf ("(rank = %d)Sending row %d from process %d\n", rank, j, next_process);
					MPI_Send (temp, (cols + 1), MPI_DOUBLE, next_process, j, MPI_COMM_WORLD);	
				}
				//printf ("(rank = %d)j = %d row reduces rows i = %d to total_rows = %d\n", rank, j, i, total_rows);
				//print_array(temp, cols);
			...
			{
				//printf ("(rank = %d)j = %d row reduces rows i = %d to total_rows = %d\n", rank, j, i, total_rows);
				//print_array(&(array[(j - rank) / size][0]), cols);
		...
		//printf ("(rank = %d)Sending row %d to process %d\n", rank, row_number, next_process);
		//print_array(&(array[i][0]), cols);
